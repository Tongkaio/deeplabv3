...
def split_images(data_dir):
    ...
    b_images = [f for f in all_images if f.startswith('B')]
    ...
    b_split = int(len(b_images) * 0.7)
    c_split = int(len(c_images) * 0.7)
    ...
    with open(train_file, 'w') as f:
        f.writelines(f'{name}\n' for name in a_images[:a_split] + b_images[:b_split] + c_images[:c_split])
